...
Without `--analyzer`, `cu analyze` picks one by file type — `prebuilt-documentSearch`
for documents, `prebuilt-imageSearch` / `-audioSearch` / `-videoSearch` otherwise — and
prints *rich Markdown*: the SDK's `to_llm_input()` Markdown (YAML front matter with
fields, then the content) plus an anchor before every section, table and figure:

```bash
cu analyze ./document.pdf
# ≡ cu analyze ./document.pdf --analyzer prebuilt-documentSearch --md-rich=coarse
```

```markdown
<!--s1-->## 2. Generation by source
<!--t0--><table>…</table>
<!--f0-->Figure 1: Q3 2025 generation by source …
```

Anchors are the array indices of the service result (`s<i>` section, `t<i>` table,
`f<i>` figure; `--md-rich=paragraph` adds `p<i>` on every paragraph). `cu resolve`
turns them back into page, bounding box and surrounding text without another
service call:

```bash
cu analyze ./document.pdf --md-rich=paragraph --json document.json
cu resolve document.json p3 --around 1      # page, bbox (inches), text, neighbours
```

Output flags are additive and each names its own destination — a bare flag means
stdout (one stream) or a `.result.*` file next to the input: `--md-rich
[PATH|coarse|paragraph]` (default), `--md [PATH]` plain Markdown, `--json [PATH]`
the complete result, `--map [PATH]` an `id → page/bbox` sidecar. Override the
analyzer with `--analyzer` (`-a` is the short form of `--analyzer`),
`CU_DEFAULT_ANALYZER`, or `cu profile set default_analyzer`. The model-free
`prebuilt-layout` gives the same structure without summary or figure descriptions:

```bash
cu analyze ./document.pdf -a prebuilt-layout
```

Every result is fetched once and then deleted on the service (`--keep-result`
keeps it); the operation id is left out of `--json` unless `--with-operation-id`.
...
`./results/invoice-01.pdf.result.rich.md`. Rich Markdown uses the
`.result.rich.md` suffix, `--md` writes `.result.md`, `--json` `.result.json`
and `--map` `.result.map.json` — combine them to get several views from one call.
