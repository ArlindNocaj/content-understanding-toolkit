...
def delete_analysis_result(client: Any, result: Any) -> Optional[bool]:
    """Best-effort ``DELETE`` of the retrieved analysis result on the service.

    *result* is the raw JSON envelope (``{"id": ..., "status": ..., "result": ...}``).
    Returns ``True`` on success, ``False`` when the service refused, ``None`` when
    there was nothing to delete.
    """
    operation_id = result.get("id") if isinstance(result, dict) else None
...
def analyze_bytes_with_usage(
    ...
    *,
    ...
    With ``delete_result`` (needs ``raw_json`` so the operation id is known) the
    stored result is deleted from the service right after it has been fetched.
    """
    ...
    if raw_json:
        ...
        if delete_result:
            deleted = delete_analysis_result(client, result)
    else:
        result = completed
