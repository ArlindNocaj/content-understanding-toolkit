...
def test_analyze_bytes_keeps_result_by_default_and_without_operation_id():
    client = _DeletingClient()
    assert analyze_bytes_with_usage(client, "a", b"x", raw_json=True).result_deleted is None
    # Without raw_json the operation id is unknown, so nothing is deleted.
    assert analyze_bytes_with_usage(client, "a", b"x", delete_result=True).result_deleted is None
