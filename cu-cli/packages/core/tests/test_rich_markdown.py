...
def test_resolve_ids_page_view_includes_geometry_and_neighbouring_pages():
    out = resolve_ids(_result(), ["t0"], page=True, pages=1)

    (item,) = out["results"]
    assert [p["page"] for p in item["pages"]] == [1, 2]
    page2 = item["pages"][1]
    assert page2["width"] == 8.5
    assert {e["id"] for e in page2["elements"]} == {"t0", "p2"}
    assert page2["markdown"].startswith("<table>")
...
def test_resolve_ids_from_coarse_map_has_bbox_but_no_paragraphs():
    id_map = build_id_map(_content(), "coarse")
    out = resolve_ids(id_map, ["t0", "p1", "#s0"])

    by_id = {r["id"]: r for r in out["results"]}
    assert by_id["t0"]["bbox"]["page"] == 2
    assert "text" not in by_id["t0"]
    assert by_id["p1"]["error"] == "unknown id"
    assert "paragraph" in by_id["p1"]["hint"]
    assert by_id["s0"]["kind"] == "section"
...
def test_resolve_ids_raw_includes_service_element():
    out = resolve_ids(_result(), ["t0"], raw=True)
    assert out["results"][0]["raw"]["rowCount"] == 1
