...
model_setup_complete=$(azd_value CU_MODEL_SETUP_COMPLETE)
...
if [ -z "$saved_endpoint" ] || [ "$profile_setup_force" = "true" ]; then
  az cu profile set endpoint "$endpoint" --name default >/dev/null
  az cu profile set default_analyzer prebuilt-layout --name default >/dev/null
  for model_name in $model_names; do
    az cu profile set "model_deployments.$model_name" "$model_name" --name default >/dev/null
  done
  if [ -n "$model_names" ]; then
    az cu defaults set --from-profile --profile default >/dev/null
  fi
