...
if (-not $savedEndpoint -or $profileSetupForce -eq 'true') {
  & az cu profile set endpoint $endpoint --name default *> $null
  & az cu profile set default_analyzer prebuilt-layout --name default *> $null
  foreach ($modelName in $modelNames) {
    & az cu profile set "model_deployments.$modelName" $modelName --name default *> $null
  }
  if ($modelNames.Count -gt 0) { & az cu defaults set --from-profile --profile default *> $null }
